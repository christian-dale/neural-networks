...
int main(int argc, char *argv[]) {
    ...
    for (int epoch = 0; epoch < 10000; epoch++) {
        for (int i = 0; i < data_length; i++) {
            double x = training_data_x[i] / (double) data_length;
            double y_true = training_data_y[i] / (double) data_length;
    ...
    printf(
        "Model test: input = %f, output = %f\n", 5.2,
        linear_model(weight,bias, 5.2 / (double) data_length) * (double) data_length
    );
}
